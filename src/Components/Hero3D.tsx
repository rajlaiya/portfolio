...
function AnimatedMobile() {
  const codeGroup = useRef<any>(null);
  const codeLines = [
    'const user = "Raj Laiya";',
    'function hello() {',
    '  return `Hi, ${user}`;',
    '}',
    'console.log(hello());',
  ...
  useFrame((state) => {
    if (codeGroup.current) {
      codeGroup.current.children.forEach((mesh: any, i: number) => {
        mesh.position.y = 0.5 - ((state.clock.elapsedTime * 0.25 + i * 0.22) % 1.2);
      });
    }
  });
  return (
    <group scale={[3.2, 3.2, 3.2]} position={[0, 0.2, 0]}>
      {/* Mobile phone body */}
      <mesh position={[0, 0, 0]}>
      ...
      <mesh position={[0, 0, 0.045]}>
        <planeGeometry args={[0.37, 0.82]} />
        <meshStandardMaterial color="#18181b" />
      </mesh>
      {/* Animated code lines as 3D text */}
      <group ref={codeGroup}>
        {codeLines.map((line, i) => (
          <Text
            key={i}
            position={[-0.16, 0.5 - i * 0.22, 0.05]}
            fontSize={0.11}
            color={i % 2 === 0 ? '#38bdf8' : '#a5b4fc'}
            anchorX="left"
            anchorY="middle"
            font="https://fonts.gstatic.com/s/firacode/v21/uU9PCBUR8oakM2BQk6mPQl5Ba_2c7A.woff"
          >
            {line}
          </Text>
        ))}
      </group>
      {/* Soft shadow under mobile */}
...
const Hero3D = () => (
  <section className="relative bg-gradient-to-br from-blue-50 via-purple-100 to-pink-50 dark:bg-gradient-to-br dark:from-gray-900 dark:via-gray-950 dark:to-black py-20 min-h-[60vh] flex items-center transition-colors duration-500 font-mono overflow-hidden" id="hero">
    ...
    <div className="container mx-auto px-6 flex flex-col md:flex-row items-center gap-10 z-10 relative">
      <div className="flex-1">
        ...
        <p className="mb-8 text-gray-700 dark:text-gray-200 max-w-xl font-['Fira_Code','JetBrains_Mono','monospace']">I build scalable web applications with modern technologies. Passionate about clean code, UI/UX, and solving real-world problems.</p>
